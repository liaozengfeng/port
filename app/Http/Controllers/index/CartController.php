...
class CartController extends Controller
{
    public function list(Request $request){
        ...
        if (empty($redis->get($request->getRequestUri()))) {
            ...
            $news_info=NewsModel::join("news_classify","news.c_id","=","news_classify.c_id")->join("admin","admin.admin_id","=","news.admin_id")->where("news.c_id",$c_id)->paginate(10);
            echo view("index.cart.list",['nav_info'=>$nav_info,"link_info"=>$link_info,"adv_info"=>$adv_info,"sli_info"=>$sli_info,"cart_info"=>$cart_info,"news_info"=>$news_info]);
